...
> **Formateo por edición (hook de Claude Code): NO habilitado.** Formatear vía Docker en cada edición añade ~5-10 s y depende de `bash` en el PATH (el entorno es PowerShell). El formato se atrapa igual en el **pre-commit (lefthook)** y en la **CI**, así que el script auxiliar se eliminó. Si más adelante se instala Go localmente, conviene un hook nativo (`gofumpt`).
...
- **Versión fijada `v2.11.0`** en `.golangci.yml`, `lefthook.yml` y `ci.yml`. Si la cambias, cámbiala en los tres.
