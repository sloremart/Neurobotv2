...
func systemPrompt(req Request) string {
	...
	b.WriteString("\nTu PRIORIDAD es reconocer el dato en lo que el paciente escribió, aunque lo diga ")
	b.WriteString("en palabras, con sinónimos, abreviado o mal escrito (p. ej. 'cédula', 'cedula de ")
	b.WriteString("ciudania', 'la amarilla' → Cédula de Ciudadanía). Si logras reconocerlo → ok=true y ")
	b.WriteString("v = EXACTAMENTE el valor que el sistema espera según lo de arriba (el número/código ")
	b.WriteString("de la opción), NUNCA las palabras del paciente.\n")
	b.WriteString("Solo si de verdad no puedes reconocerlo → ok=false y escribe en m un mensaje conversacional y amable, con TUS ")
